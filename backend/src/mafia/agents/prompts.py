...
def _speak_strategy_hint(state: GameState, actor: Player) -> str:
    ...
    if state.day_number == 1:
        return (
            "**첫날 자기소개 안내**: 지금은 라운드로빈 자기소개 시간입니다. "
            "페르소나(직업·성격·말투)를 드러내고, 이 게임을 어떻게 임할지 태도만 짧게 보이세요.\n"
            "- **누구도 지목·의심하지 마세요** — 아직 아무 정보가 없습니다.\n"
            "- 이전에 발언한 사람이 적으면(또는 없으면), '조용하다/수상하다'는 식의 가짜 관찰을 "
            "절대 만들어내지 마세요.\n"
            "- 능력 공개도 보류 (밤이 한 번도 지나지 않았으므로 줄 정보가 없습니다).\n"
            "의심·지목·반응은 자유발언과 둘째 날부터 시작됩니다."
        )
